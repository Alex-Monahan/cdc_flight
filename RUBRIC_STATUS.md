...
| 4.5 | Errors must not hang or lock | **4** | Every engine, source, catalog, destination-retirement, phase-sink, and commit wait has an explicit bound and failure outcome; the real concurrent and idle probes complete. A complete adversarial hang matrix for every individual wait remains unproven. |
