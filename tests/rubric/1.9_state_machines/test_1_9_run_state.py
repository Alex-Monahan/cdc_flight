...
def test_the_supervisor_no_longer_assigns_the_outcome_by_hand():
    ...
    path = Path(__file__).resolve().parents[3] / "src" / "cdc_flight" / "supervisor.py"
...
def test_the_pipeline_reports_the_retirement_after_it_has_happened():
    ...
    """
    source = (
        Path(__file__).resolve().parents[3] / "src" / "cdc_flight" / "pipeline.py"
    ).read_text()
...
# --------------------------------------------------------------------------- #
TEARDOWN_DRIVER = Path(__file__).resolve().parents[2] / "support" / "teardown_order_driver.py"
